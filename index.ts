...
app.event("reaction_removed", async ({ event }) => {
    const channelInEventItem = (item: Record<string, unknown>): item is { channel: string } => "channel" in item
    if (channelInEventItem(event.item)) {
        if (event.reaction === "upvote") {
...
async function modifyKarma(amount: number, user: string, channel: string) {
...
app.command("/karma-global-user", async ({ command, ack, client }) => {
    ...
    const match = command.text.match(/<@(.*)\|.*>/i)
    if (match) {
...
app.command("/karma-channel-personal", async ({ command, ack, client }) => {
    ...
    client.chat.postEphemeral({
        ...
        text: `You have a total of ${
            karma ? karma.karmaForChannel : 0
        } Karma in <#${command.channel_id}> ${getRandomEmoji()}`,
    })
})
